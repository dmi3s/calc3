...
项目处于初期阶段：主要基础模块已经实现，但最终目标（输出逆波兰表达式、计算表达式的值）
仍在规划之中。
...
- **访问者（Visitor）** — `src/ast.c3`：用于遍历树的访问者模式；目前有一个
  打印节点的演示访问者。
- **语法分析器（Parser）** — `src/parser.c3`：按照上面的文法进行递归下降，
  遵循优先级和结合性；构建 AST，支持一元运算符和括号。错误会连同描述和位置一起返回。
- **测试** — `test/`：针对词法分析器、AST 和语法分析器的单元测试（27 个测试）。
  运行方式：`c3c test calc3`。

尚未完成（但已规划）：计算表达式的值，并以逆波兰表示法输出结果。暂不规划：
词法分析器/语法分析器在出错后的恢复。
