...
Проект на начальной стадии: реализованы основные строительные блоки и вывод выражения
в обратной польской записи; впереди осталось вычисление значения выражения.
...
- **Тесты** — `test/`: модульные тесты лексера, AST и парсера (37 тестов).
  Запуск: `c3c test calc3`.

Ещё не сделано (но планируется): вычисление значения выражения. Не планируется:
восстановление лексера/парсера после ошибок.
...
│   └── main.c3                    ── Точка входа: демонстрационный запуск
├── test/                          ── Тесты
